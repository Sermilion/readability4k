...
class NotebookcheckRealTest :
  FunSpec({

    test("should extract full review content from real notebookcheck HTML") {
      runTest {
        val htmlResource = this::class.java.classLoader.getResourceAsStream(
          "notebookcheck-real.html",
        )
        ...
        val introIndex = content.indexOf("affordable gaming smartphone from Vivo")
        val q3ChipIndex = content.indexOf("Q3 gaming chip")
        val pubgIndex = content.indexOf("PUBG Mobile")

        println("Intro index: $introIndex")
        println("Q3 chip index: $q3ChipIndex")
        println("PUBG index: $pubgIndex")

        introIndex shouldBeLessThan q3ChipIndex
        q3ChipIndex shouldBeLessThan pubgIndex
      }
    }
  })
